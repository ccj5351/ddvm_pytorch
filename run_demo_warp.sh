...
if [ "$flag" = true ]; then
    ...
    if [ $DATA_EVAL = "kitti" ]; then
        RESULT_DIR="/home/ccj/code/tpk2scene/results_nfs/ddvm/imagenunet/kt15_warp_tile"
    elif [ $DATA_EVAL = "sintel" ]; then
        RESULT_DIR="/home/ccj/code/tpk2scene/results_nfs/ddvm/imagenunet/sintel_warp_tile"
    fi
    CUDA_VISIBLE_DEVICES=$GPU_ID python3 evaluate_diffusers_warprefine.py \
...
if [ "$flag" = true ]; then
    ...
    if [ $DATA_EVAL = "kitti" ]; then
        RESULT_DIR="/home/ccj/code/tpk2scene/results_nfs/ddvm/raftunet/kt15_warp_tile"
    elif [ $DATA_EVAL = "sintel" ]; then
        RESULT_DIR="/home/ccj/code/tpk2scene/results_nfs/ddvm/raftunet/sintel_warp_tile"
    fi
    CUDA_VISIBLE_DEVICES=$GPU_ID python3 evaluate_diffusers_warprefine.py \
        --pipeline_path $CKPT_PTH \
        --validation_data $DATA_EVAL \
